...
$translations = [
    'en' => [
        ...
        'alert_overdue_title' => 'Immediate Action Required: Overdue Tasks Detected!',
        'alert_overdue_desc' => 'There are %d tasks that have crossed their deadlines. Please review and update their status or reassign them to prevent further delays.',
        'alert_overdue_btn' => 'View Detailed Overdue Report'
    ],
    'mr' => [
        ...
        'alert_overdue_title' => 'त्वरित कृती आवश्यक: थकीत कार्ये आढळली!',
        'alert_overdue_desc' => 'अमरावती जिल्ह्यात %d कार्ये त्यांच्या मुदती ओलांडून गेली आहेत. पुढील विलंब टाळण्यासाठी कृपया त्यांच्या स्थितीचे पुनरावलोकन आणि अद्यतन करा.',
        'alert_overdue_btn' => 'थकीत अहवाल पहा'
    ]
];
$t = $translations[$lang];
    ...
    try {
        ...
        $r = $conn->query("
            SELECT
              ...
              COUNT(CASE WHEN status  = 'Pending' AND (due_date >= CURDATE() OR due_date IS NULL) THEN 1 END) AS pending,
        ...
        $res = $conn->query("
            SELECT
              COALESCE(t.taluka_name, 'Unknown')                           AS taluka,
              COUNT(*)                                                     AS total,
              COUNT(CASE WHEN tasks.status = 'Completed' THEN 1 END)       AS completed,
              COUNT(CASE WHEN tasks.status = 'Pending' AND (tasks.due_date >= CURDATE() OR tasks.due_date IS NULL) THEN 1 END) AS pending,
              COUNT(CASE WHEN tasks.due_date < CURDATE()
                         AND tasks.status != 'Completed' THEN 1 END)       AS overdue,
              ROUND(COUNT(CASE WHEN tasks.status='Completed' THEN 1 END)
                    / NULLIF(COUNT(*),0)*100, 1)                           AS rate
            FROM tasks
            LEFT JOIN talukas t ON tasks.taluka_id = t.taluka_id
            GROUP BY tasks.taluka_id, t.taluka_name
            ORDER BY rate DESC
            LIMIT 10
        ");
        ...
        $st = $conn->prepare("
            SELECT
              ...
              COUNT(CASE WHEN tasks.due_date<CURDATE()
                         AND tasks.status!='Completed' THEN 1 END)         AS overdue
            FROM tasks
            LEFT JOIN villages v ON tasks.village_id = v.village_id
            WHERE tasks.taluka_id = ?
            GROUP BY tasks.village_id, v.village_name
 ...
 */
function getVillageStats(mysqli $conn, int $level, int $talukaId, int $villageId): array {
...
$vilData    =           getVillageStats($conn, $level, $sTalukaId, $sVillageId);
...
function inProgress(int $active, int $pending, int $overdue = 0): int {
    return max(0, $active - $pending - $overdue);
}
...
<div class="flex-1 flex flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900 p-6 sm:p-8">
        ...
        if (isset($_SESSION['msg'])):
            $msg = $_SESSION['msg'];
            $msgType = $_SESSION['msgType'] ?? 'success';
            unset($_SESSION['msg']);
            unset($_SESSION['msgType']);
        ?>
        <div id="phpAlert" class="mb-6 flex items-start gap-3 p-4 <?= $msgType === 'success' ? 'bg-govgreen-50 dark:bg-green-900/20 border border-govgreen-100 dark:border-green-800 text-govgreen-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300' ?> rounded-xl animate-in">
            <i data-lucide="<?= $msgType === 'success' ? 'check-circle-2' : 'alert-circle' ?>" class="w-5 h-5 flex-shrink-0 mt-0.5"></i>
            <div>
                <p class="text-sm font-medium"><?= $msg ?></p>
            </div>
            <button onclick="document.getElementById('phpAlert').remove()" class="ml-auto opacity-70 hover:opacity-100">
        ...
        if ($level === 1) {
            $currentOverdueCount = $distData['overdue'];
        } elseif ($level === 2) {
            $currentOverdueCount = $talData['overdue'];
        } else {
            $currentOverdueCount = $vilData['overdue'];
        }
        
        if ($currentOverdueCount > 0):
        ?>
        <div class="mb-6 relative overflow-hidden rounded-xl border border-red-200 dark:border-red-900 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-lg shadow-red-500/5 dark:shadow-red-500/2 animate-in fade-in slide-in-from-top-4 duration-300">
            <!-- Decorative soft red light beam background -->
            <div class="absolute -right-20 -top-20 w-48 h-48 rounded-full bg-red-500/10 dark:bg-red-500/5 blur-3xl pointer-events-none"></div>
            
            <div class="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 w-12 h-12 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-full flex items-center justify-center shadow-inner">
                        <i data-lucide="alert-octagon" class="w-6 h-6 text-red-600 dark:text-red-400 animate-pulse"></i>
                    </div>
                    <div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            <?= htmlspecialchars($t['alert_overdue_title']) ?>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800">
                                <?= $lang === 'en' ? 'URGENT' : 'तातडीचे' ?>
                            </span>
                        </h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1 max-w-2xl leading-relaxed">
                            <?= sprintf($t['alert_overdue_desc'], $currentOverdueCount) ?>
                        </p>
                    </div>
                </div>
                
                <div class="flex items-center gap-3 self-end md:self-center">
                    <a href="overdue_report.php?lang=<?= $lang ?>" 
                       class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-semibold rounded-lg text-white bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all shadow-sm shadow-red-500/20" 
                       style="text-decoration: none;">
                        <i data-lucide="file-text" class="w-4 h-4 mr-2"></i>
                        <?= htmlspecialchars($t['alert_overdue_btn']) ?>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8">
            ...
            <div class="mt-4 md:mt-0 flex items-center space-x-3 flex-wrap gap-y-2">
                ...
                <a href="create_task.php?lang=<?= $lang ?>" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm
                               text-sm font-medium rounded-md text-white
                               bg-navy-600 hover:bg-navy-700 focus:outline-none transition-colors" style="text-decoration: none;">
        ...
        <div class="mb-10">
            ...
            <div id="sec-dist" class="sec-body">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                    <?php
                    $dkpi = [
                        [$t['kpi_active'],    $distData['active'],    'layers',       'blue',   'trending-up',   '+12%', true, 'active'],
                        [$t['kpi_pending'],   $distData['pending'],   'clock',        'orange', 'trending-down', '-4%',  false, 'pending'],
                        [$t['kpi_completed'], $distData['completed'], 'check-circle', 'green',  'trending-up',   '+24%', true, 'completed'],
                        [$t['kpi_overdue'],   $distData['overdue'],   'alert-octagon','red',    'alert-triangle', $lang === 'en' ? '12 Action Req' : '१२ कृती आवश्यक', false, 'overdue'],
                    ];
                    foreach ($dkpi as [$label,$val,$icon,$clr,$trendIcon,$trendTxt,$trendUp,$metric]):
                    ?>
                    <div class="kpi-card bg-white dark:bg-slate-800 overflow-hidden shadow-sm
                                rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-all"
                         data-metric="<?= $metric ?>">
                        <div class="p-5">
        ...
        <div class="mb-10">
            ...
            <div id="sec-tal" class="sec-body">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                    ...
                    foreach ($tkpi as [$label,$val,$icon,$clr,$tIcon,$tTxt,$tUp,$metric]):
                    ?>
                    <div class="kpi-card bg-white dark:bg-slate-800 overflow-hidden shadow-sm
                                rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-all"
                         data-metric="<?= $metric ?>">
        ...
        <div class="mb-12">
            ...
            <div id="sec-vil" class="sec-body">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                    ...
                    foreach ($vkpi as [$label,$val,$icon,$clr,$tIcon,$tTxt,$tUp,$metric]):
                    ?>
                    <div class="kpi-card bg-white dark:bg-slate-800 overflow-hidden shadow-sm
                                rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-all"
                         data-metric="<?= $metric ?>">
                            ...
                            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors task-row"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="respond_task.php?task_id=<?= $task['task_id'] ?>&lang=<?= $lang ?>"
                                           class="inline-flex text-navy-600 dark:text-blue-400 hover:text-navy-900
...
function buildAllCharts(isDark) {
    ...
    charts.dDonut = new ApexCharts(
        ...
        donutOpts(
            [<?= inProgress($distData['active'],$distData['pending'],$distData['overdue']) ?>,
             <?= $distData['pending'] ?>,
             <?= $distData['completed'] ?>,
             <?= $distData['overdue'] ?>],
    ...
    charts.tDonut = new ApexCharts(
        ...
        donutOpts(
            [<?= inProgress($talData['active'],$talData['pending'],$talData['overdue']) ?>,
             <?= $talData['pending'] ?>,
             <?= $talData['completed'] ?>,
             <?= $talData['overdue'] ?>],
            [<?= json_encode($t['chart_in_progress']) ?>,
    ...
    charts.vDonut = new ApexCharts(
        ...
        donutOpts(
            [<?= inProgress($vilData['active'],$vilData['pending'],$vilData['overdue']) ?>,
             <?= $vilData['pending'] ?>,
             <?= $vilData['completed'] ?>,
             <?= $vilData['overdue'] ?>],
            [<?= json_encode($t['chart_in_progress']) ?>,
...
<div id="reportDetailsModal" class="fixed inset-0 z-50 hidden overflow-y-auto animate-fade-in" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-slate-200 dark:border-slate-700 animate-in">
            <!-- Header -->
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-850 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white" id="reportModalTitle">
                            Detailed Report
                        </h3>
            ...
            <div class="px-6 py-3 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row gap-3 items-center justify-between bg-white dark:bg-slate-800">
                <div class="relative w-full sm:max-w-xs">
                    ...
                    <input type="text" id="reportModalSearch" oninput="filterReportModalTasks()" placeholder="<?= htmlspecialchars($lang === 'en' ? 'Search tasks...' : 'कार्ये शोधा...') ?>" class="w-full pl-9 pr-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-navy-500 focus:border-navy-500">
                </div>
                <div class="flex gap-2 w-full sm:w-auto">
                    <button onclick="exportReportToCSV()" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm">
                        <i data-lucide="download-cloud" class="w-4 h-4 mr-2 text-slate-500"></i>
                        <span><?= $lang === 'en' ? 'Export CSV' : 'CSV निर्यात करा' ?></span>
                    </button>
                </div>
            </div>
            ...
            <div class="px-6 py-4 max-h-[50vh] overflow-y-auto bg-slate-50 dark:bg-slate-900/10">
                ...
                <div id="reportModalContent" class="hidden">
                    <div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800">
                        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                            <thead class="bg-slate-50 dark:bg-slate-900/50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Task Info</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Assigned Officer</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Location</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Priority</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Timeline</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700" id="reportModalTableBody">
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center rounded-b-2xl">
                <span class="text-xs font-medium text-slate-500 dark:text-slate-400" id="reportModalFooterStats">Showing 0 tasks</span>
                <button onclick="closeReportModal()" class="px-4 py-2 bg-navy-600 hover:bg-navy-700 dark:bg-navy-500 dark:hover:bg-navy-600 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors focus:outline-none">
...
function openReportModal(metric, extraParams = {}) {
    ...
    if (currentLang === 'mr') {
        ...
        metricLabel = mrMetrics[metric] || metric;
    }
    
    titleSpan.innerHTML = `<i data-lucide="clipboard-list" class="w-5 h-5 text-navy-600 dark:text-blue-400 inline-block mr-2 align-middle"></i> ` + 
        (currentLang === 'en' ? `Detailed Report: ${metricLabel} Tasks` : `तपशीलवार अहवाल: ${metricLabel} कार्ये`);
...
function renderReportModalTasks(tasks) {
    ...
    tasks.forEach(task => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3 whitespace-nowrap">
                <span class="text-xs font-semibold ${getPriorityCss(task.priority)}">
                    ${task.priority}
                </span>
            </td>
            <td class="px-4 py-3 whitespace-nowrap">
                <div class="text-sm text-slate-900 dark:text-white">${task.due_date_formatted}</div>
                <div class="text-xs ${task.status === 'Overdue' ? 'text-red-600 dark:text-red-400 font-medium' : 'text-slate-500 dark:text-slate-400'}">${task.timeline_info || ''}</div>
            </td>
            <td class="px-4 py-3 whitespace-nowrap">
                <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full border ${getStatusCss(task.status)}">
                    ${task.status}
                </span>
            </td>
        `;
    ...
    document.getElementById('reportModalFooterStats').textContent = statsText;
}
...
function filterReportModalTasks() {
    ...
    const filtered = currentReportTasks.filter(task => {
        return (task.title || '').toLowerCase().includes(val) ||
               ...
               (task.taluka_name || '').toLowerCase().includes(val) ||
               (task.village_name || '').toLowerCase().includes(val);
    });
...
function exportReportToCSV() {
    ...
    let csv = [];
    csv.push(['Task ID', 'Title', 'Description', 'Status', 'Due Date', 'Priority', 'Category', 'Assigned To', 'Designation', 'Taluka', 'Village'].join(','));
    ...
    link.setAttribute("download", `Amravati_Connect_Report_${Date.now()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}

// Add cursor pointer and click handler to KPI cards
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.kpi-card[data-metric]').forEach(card => {
        card.addEventListener('click', () => {
            const metric = card.getAttribute('data-metric');
            if (metric === 'overdue') {
                window.location.href = `overdue_report.php?lang=${currentLang}`;
            } else {
                openReportModal(metric);
            }
        });
    });
